...
declare
  ...
  lease_row public.monthly_v7_entity_leases%rowtype; operation_row public.monthly_v7_operations%rowtype;
...
begin
  ...
  request_hash:=encode(digest(convert_to(jsonb_build_object('command','delete_module','module_id',p_module_id,
    ...
    'lease_id',p_lease_id,'fencing_token',p_fencing_token)::text,'UTF8'),'sha256'),'hex');
  insert into public.monthly_v7_operations(operation_id,workspace_id,actor_user_id,command_type,entity_type,entity_id,request_hash,status)
  ...
  if operation_row.status in ('COMMITTED','REJECTED') then return operation_row.result; end if;
  select * into lease_row from public.monthly_v7_entity_leases where workspace_id=workspace_row.id and entity_type='module' and entity_id=p_module_id for update;
  if workspace_row.authority_state<>'NORMALIZED_ACTIVE' then response:=jsonb_build_object('ok',false,'error','AUTHORITY_NOT_ACTIVE');
  elsif not found or lease_row.expires_at<=now() or lease_row.lease_id<>p_lease_id or lease_row.fencing_token<>p_fencing_token
     or lease_row.holder_user_id<>user_row.id or lease_row.holder_user_session_id<>p_user_session_id or lease_row.client_session_id<>btrim(p_client_session_id) then response:=jsonb_build_object('ok',false,'error','LEASE_LOST');
  ...
  update public.monthly_v7_entity_leases set heartbeat_at=now(),expires_at=now() where workspace_id=workspace_row.id and entity_type='module' and entity_id=p_module_id;
...
revoke execute on function public.monthly_v7_delete_module(text, uuid, text, uuid, uuid, bigint, bigint, uuid, bigint) from public;
...
grant execute on function public.monthly_v7_delete_module(text, uuid, text, uuid, uuid, bigint, bigint, uuid, bigint) to authenticated;
