...
test('page、config、assets、core、client 與 V7 宣告同一 build ID 並提供 mixed-build 安全重載', async () => {
  const buildId = '7.6.4';
  const html = await readFile(join(root, 'index.html'), 'utf8');
  assert.match(html, /MONTHLY_REPORT_PAGE_BUILD = '7\.6\.4'/);
...
test('數據管理頁呈現密碼權限矩陣與分層空間統計，不把本機月報冒充 Supabase 用量', async () => {
  ...
  assert.match(html, /專題報告內容與快照邏輯量/);
  assert.match(html, /本機歷史月報（IndexedDB，不計入 Supabase）/);
  assert.match(html, /listTopicReportsForStorage/);
