...
test('月報只在數據管理右側新增外部專題入口且mirror逐byte一致', async () => {
  ...
  const marker = '<a id="topicReportsEntry" class="v1-tab-btn v1-topic-entry" href="./topic-reports.html?v=1.12.0" target="topic-reports"';
...
test('專題清單頁只載入topic assets且具備新增、刷新、歷史清單與清單PDF入口', async () => {
  ...
  assert.match(sources, /report-assets-storage\.js/);
...
test('專題編輯頁是獨立窗口且完整列出編輯、模塊、保存、Excel、PDF及重置控制', async () => {
  ...
  [
    ...
    '進度卡', '三色卡', '趨勢圖', '附件', '直向', '橫向'
  ].forEach((label) => assert.match(html, new RegExp(label)));
  assert.doesNotMatch(html, /版型：單欄／雙欄/);
  const metaStart = html.indexOf('<div class="topic-editor-meta">');
  const metaEnd = html.indexOf('</div>\n      <div id="topicModules"', metaStart);
  const meta = html.slice(metaStart, metaEnd);
  assert.ok(metaStart >= 0 && metaEnd > metaStart, 'missing editor meta region');
  ['topicReportTitle', 'topicReportDate', 'topicModeBadge', 'topicLeaseNotice', 'topicAcquireEdit']
    .forEach((id) => assert.match(meta, new RegExp(`id=["']${id}["']`), `${id}應與名稱日期同區`));
...
test('topic資產版本完全一致且list/editor啟動前執行混版fail-closed檢查', async () => {
  ...
  assert.equal(core.BUILD_ID, '1.12.0');
  ...
  for (const file of ['topic-reports.html', 'topic-report-editor.html']) {
    ...
    const versions = [...html.matchAll(/(?:report-assets-storage|topic-reports-(?:core|client)|topic-reports-page|topic-report-editor)\.js\?v=([^"']+)/g)]
      .map((match) => match[1]);
    assert.ok(versions.length >= 4, `${file}應載入四個versioned topic資產`);
    assert.deepEqual([...new Set(versions)], ['1.12.0']);
  }
});

test('所有新增topic JavaScript可由Node parser解析', async () => {
  const files = [
    'report-assets-storage.js',
