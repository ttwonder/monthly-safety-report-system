...
7. 多人協作：打開系統會自動同步 Supabase；登入後修改會自動保存，也可使用頂部「同步最新」和「保存修改」。
8. 管理帳號 / Supabase 設定 / JSON 備份：進入「數據管理」。
9. 正式部署共用設定：`supabase-config.js` 內放 Supabase URL、anon public key、Workspace Key，所有使用者打開同一網址會讀寫同一份雲端資料。
